...
namespace sh_impl
{
    ...
    template<int L, int M> struct sh_expr;

#define DEFINE_SH(L, M, COEF, EXPR) \
    template<>\
    struct sh_expr<L, M> \
    { \
        static float eval([[maybe_unused]] const float x,[[maybe_unused]] const float y ,[[maybe_unused]] const float z) noexcept \
        { \
            [[maybe_unused]] constexpr float pi = 3.1415926f; \
            static const float C = (COEF); \
            return C * (EXPR); \
        } \
    }

    DEFINE_SH(0, 0, 0.5f * std::sqrt(1 / pi), 1);

    DEFINE_SH(1, -1, std::sqrt(3 / (4 * pi)), y);
    DEFINE_SH(1, 0, std::sqrt(3 / (4 * pi)),  z);
    DEFINE_SH(1, 1, std::sqrt(3 / (4 * pi)),  x);

    DEFINE_SH(2, -2, (0.5f)  * std::sqrt(15 / pi), x * y);
    DEFINE_SH(2, -1, (0.5f)  * std::sqrt(15 / pi), y * z);
    DEFINE_SH(2, 0,  (0.25f) * std::sqrt(5 / pi),  -x * x - y * y + 2 * z * z);
    DEFINE_SH(2, 1,  (0.5f)  * std::sqrt(15 / pi), z * x);
    DEFINE_SH(2, 2,  (0.25f) * std::sqrt(15 / pi), x * x - y * y);

    DEFINE_SH(3, -3, 0.25f * std::sqrt(35 / (2 * pi)), (3 * x * x - y * y) * y);
    DEFINE_SH(3, -2, 0.5f  * std::sqrt(105 / pi),      x * y * z);
    DEFINE_SH(3, -1, 0.25f * std::sqrt(21 / (2 * pi)), y * (4 * z * z - x * x - y * y));
    DEFINE_SH(3, 0,  0.25f * std::sqrt(7 / pi),        z * (2 * z * z - 3 * x * x - 3 * y * y));
    DEFINE_SH(3, 1,  0.25f * std::sqrt(21 / (2 * pi)), x * (4 * z * z - x * x - y * y));
    DEFINE_SH(3, 2,  0.25f * std::sqrt(105 / pi),      (x * x - y * y) * z);
    DEFINE_SH(3, 3,  0.25f * std::sqrt(35 / (2 * pi)), (x * x - 3 * y * y) * x);

	DEFINE_SH(4, -4, 0.75f * std::sqrt(35 / pi), x* y* (x* x - y * y));
	DEFINE_SH(4, -3, 0.75f * std::sqrt(35 / (2 * pi)), (3 * x * x - y * y)* y* z);
	DEFINE_SH(4, -2, 0.75f * std::sqrt(5 / pi), x* y* (7 * z * z - 1));
	DEFINE_SH(4, -1, 0.75f * std::sqrt(5 / (2 * pi)), y* z* (7 * z * z - 3));
    DEFINE_SH(4, 0,  3.f/16.f * std::sqrt(1 / pi), (35 * (z * z) * (z * z) - 30 * z * z + 3));
    DEFINE_SH(4, 1,  0.75f * std::sqrt(5 / (2 * pi)), x * z * (7 * z * z - 3));
    DEFINE_SH(4, 2,  3.f/8.f * std::sqrt(5 / pi), (x * x - y * y) * (7 * z * z - 1));
    DEFINE_SH(4, 3,  0.75f * std::sqrt(35 / (2 * pi)), (x * x - 3 * y * y) * x * z);
    DEFINE_SH(4, 4,  3.f / 16.f * std::sqrt(35 / pi), x * x * (x * x - 3 * y * y) - y * y * (3 * x * x - y * y));

#undef DEFINE_SH

/*
    template<int L> struct rotate_sh_coefs_impl;

    struct rotate_sh_coefs_impl<0>
    {
        static void call(const glm::mat3&, float*) noexcept
        {
            
        }
    };

    struct rotate_sh_coefs_impl<1>
    {
        static void call(const glm::mat3 &rot, float *coefs) noexcept
        {
            float PI = 3.1415926f;
            static const float INV_C = std::sqrt(4 * PI / 3.f);
...
namespace spherical_harmonics
{
    ...
    auto eval(const float&x,const float&y,const float&z) noexcept
    {
        return sh_impl::sh_expr<L, M>::eval(x,y,z);
    }
...
//	using sh_func_t = float* (const float& x, const float& y, const float& z) noexcept;

    typedef float (*sh_func_t)(const float& x, const float& y, const float& z) noexcept;

    sh_func_t *linear_table() noexcept
    {
        static sh_func_t table[] =
        {
			&spherical_harmonics::eval<0, 0>,

			&spherical_harmonics::eval<1, -1>,
			&spherical_harmonics::eval<1,  0>,
			&spherical_harmonics::eval<1,  1>,

			&spherical_harmonics::eval<2, -2>,
			&spherical_harmonics::eval<2, -1>,
			&spherical_harmonics::eval<2,  0>,
			&spherical_harmonics::eval<2,  1>,
			&spherical_harmonics::eval<2,  2>,
			
			&spherical_harmonics::eval<3, -3>,
			&spherical_harmonics::eval<3, -2>,
			&spherical_harmonics::eval<3, -1>,
			&spherical_harmonics::eval<3,  0>,
			&spherical_harmonics::eval<3,  1>,
			&spherical_harmonics::eval<3,  2>,
			&spherical_harmonics::eval<3,  3>,
			
			&spherical_harmonics::eval<4, -4>,
			&spherical_harmonics::eval<4, -3>,
			&spherical_harmonics::eval<4, -2>,
			&spherical_harmonics::eval<4, -1>,
			&spherical_harmonics::eval<4,  0>,
			&spherical_harmonics::eval<4,  1>,
			&spherical_harmonics::eval<4,  2>,
			&spherical_harmonics::eval<4,  3>,
			&spherical_harmonics::eval<4,  4>,
