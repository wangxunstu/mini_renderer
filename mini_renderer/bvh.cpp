	...
	int            leafSizeThreshold)
{
	...
	while (!tasks.empty())
	{
		...
		if (n <= leafSizeThreshold)
		{
			...
			auto leaf = new BuildNode();//arena.create<BuildNode>();
			leaf->aabb = allBound;
			leaf->left = nullptr;
			leaf->right = nullptr;
...
bool hasIntersectionWithTriangle(
	...
	const glm::vec3& A,
	const glm::vec3& B_A,
	const glm::vec3& C_A) noexcept
{
	const glm::vec3 s1 = cross(r.d, C_A);
	const float div = dot(s1, B_A);
	if (div == 0.0f)
		return false;
	const float invDiv = 1 / div;

	const glm::vec3 o_A = r.o - A;
	const float alpha = dot(o_A, s1) * invDiv;
	if (alpha < 0 || alpha > 1)
		return false;

	const glm::vec3 s2 = cross(o_A, B_A);
...
BVH BVH::create(const glm::vec3* triangle_vertices, int triangle_count)
{
	...
	auto [root, node_count] = buildLinkedBVH(
		build_triangles.data(), triangle_count, 4);
...
{
	const glm::vec3 invDir = { 1 / ray.d.x, 1 / ray.d.y, 1 / ray.d.z };
...
{
	auto r = ray;
	const glm::vec3 invDir = { 1 / r.d.x, 1 / r.d.y, 1 / r.d.z };
